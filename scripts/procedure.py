...
class Procedure:
    ...
    def __init__(self, config, df_in=None, file_name=None, name_suffix="", folder=None,
        ...
        self.results = list()
        if self.save_data:
            write_file(self.df, "data" + self.file_name, # save the dataset
    ...
    def _write_results(self, ix=None):
        ...
        write_file(pred_all , "all_pred" + self.file_name,
                  path=self.folder, shorten=False)
        ...
        #               "mean_iter" + self.file_name, path=self.folder)
        #     write_file(pd.concat([performance_across_se, iters], axis=1),
        #               "se_iter" + self.file_name, path=self.folder)
        #     write_file(pd.concat([output_metric_append, iters], axis=1),
        #               "mean_append" + self.file_name,
        #               path=self.folder)
        #     if self.config.exp_year_split is None:
        #         write_file(pd.concat([output_metric_fold_mean, iters], axis=1),
        #                   "mean_fold" + self.file_name,
        #                   path=self.folder)
        #         write_file(pd.concat([output_metric_fold_se, iters], axis=1),
        #                   "se_fold" + self.file_name,
        #                   path=self.folder)
        # else:
        #     write_file(pd.concat([performance_across_mean, iters], axis=1),
        #               "mean_iter_ix" + self.file_name,
        #               path=self.folder)
        #     write_file(pd.concat([performance_across_se, iters], axis=1),
        #               "se_iter_ix" + self.file_name,
        #               path=self.folder)
        #     write_file(pd.concat([output_metric_append, iters], axis=1),
        #               "mean_append_ix" + self.file_name,
        #               path=self.folder)
    ...
    def _write_hyper_param(self, algo):
        ...
        try:
            ...
            for p in params:
                ...
                write_file(pd.DataFrame(out), "hyper_" + algo + self.file_name,
                  path=self.folder, shorten=False)
        except:
            pass
    ...
    def _write_shapley(self, model_name=None, **kwargs):
        ...
        write_file(shap_out_mean, file_name="shapley_mean_" + model_name \
                  + self.file_name, path=self.folder, shorten=False)
        ...
        write_file(shap_out_append, file_name="shapley_append_" + model_name \
                  + self.file_name, path=self.folder,
                  shorten=False)



        # save Shapley interaction
        if self.config.exp_shapley_interaction:
            shapley_inter_out = np.vstack([self.results[i]["shapley_interaction"].loc[model_name,:,: ].values for i in range(len(self.results))])
            shapley_inter_out = pd.DataFrame(shapley_inter_out, columns = self.results[0]["shapley_interaction"].coords["combination"].to_numpy())
            
            shapley_inter_out["pred"] = pred_append
            shapley_inter_out["year"] = np.tile(self.df["year"], nrep)
            shapley_inter_out["iso"] = np.tile(self.df["iso"], nrep)
            shapley_inter_out["crisis"] = np.tile(self.Y, nrep)

            write_file(shapley_inter_out, file_name="shapley_interaction_" + model_name \
                            + self.file_name, path=self.folder,
                            shorten=False)
    ...
    def _save_pickle(self, file_name=None, keep_models=False, keep_data=False):
        ...
        if file_name is None:
            file_name = "pickle" + self.file_name + "_" + str(np.random.randint(100000000))
