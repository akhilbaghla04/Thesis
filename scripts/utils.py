...
def make_level_change(data_input, variables, type, horizon=10, norm = False):
    names_out = []
    data = data_input.copy()
    ...
    for var in variables:
        ...
        #     data[varname] = np.nan
        #     data_mad = pd.DataFrame(data_grouped.apply(mov_ave_diff, var, horizon), columns=[varname])
        #     for iso in data_mad.index.values:
        #         data.loc[data.iso == iso, varname] = data_mad.loc[iso, varname]
        if type == "ham":
            varname = var + '_ham'
            if norm:
                varname = varname + "_norm"
            data[varname] = np.nan
            data_ham = pd.DataFrame(data_grouped.apply(hamilton_filter, var, 2, 4, norm), columns=[varname])
        ...
        if type == "hp":
            ...
            data_hp = pd.DataFrame(data_grouped.apply(hp_filter_wrapper, var, norm), columns=[varname])

            for iso in data_hp.index.values:
                data.loc[data.iso == iso, varname] = data_hp.loc[iso, varname]
...
def hp_filter_wrapper(group, col, norm = False, output = "cycle"):
    x = group[col].values
    # y = pd.Series(x).interpolate(method="spline", order = 3).values  # we cannot interpolate first and last values. Therefore we filter for nans again below
    y = pd.Series(x).interpolate(method = "linear").values # Split of order 2 and 3 performed worse
    yt = y.transpose().reshape((len(y), 1))
    out = np.empty((len(x)))
    out[:] = np.nan
    hp_out = hp_filter_serial(yt[~np.isnan(y),], norm = norm, output = output)
    out[~np.isnan(y)] = np.squeeze(hp_out)
    out = np.squeeze(out)
    out[np.isnan(x)] = np.nan # remove values that are missing in orginal data
    return out

def hamilton_filter(group, col, h=2, p=4, norm = False, output="cycle"):  # output: "x", "trend", "cycle", "random" and "all"
    # name used by Miao: yth_filter
    ...
    x = group[col].values
    if norm:
        x = (x - np.nanmin(x))/(np.nanmax(x) -np.nanmin(x))   # normalise between [0 and 1]
    # note: Hamilton used 100 times x's logrithm in his employment data, however, this is commented out because we log the data before
    ...
    if trend is not None:  # if dataframe passed is not full of nans
        # Get the cycle which is simple the original series subtracts the trend
        cycle = x - trend
        # Get the random walk series which is simply the difference between the original series and the h look back
        df_x = pd.DataFrame(x)
        df_x_h = df_x.shift(h)
        random = df_x - df_x_h
    else:
        ...
        cycle = x
        random = x
    # Return required series in result, if all is selected then all results are returned in a data frame
    if (output == "x"):
        return x
    elif (output == "trend"):
        return trend
...
def hp_filter_serial(y, lam = 100, norm = False, output = "cycle"):
    # filter works! Produces exactly the same results as the Matlab implementation
    T = len(y)  # size of the input
    if norm:
        y = (y - np.nanmin(y))/(np.nanmax(y) -np.nanmin(y))

    x1 = np.array([1.0 + lam, - 2 * lam, lam])
    x2 = np.array([- 2.0 * lam, 1 + 5.0 * lam, -4.0 * lam, lam])
    x3 = np.array([lam, -4.0 * lam, 1.0 + 6.0 * lam, -4.0 * lam, lam])
    # tbc
    x1_rev = np.flip(x1)
    x2_rev = np.flip(x2)
    ...
    y_trend[0:2] = y[0:2]
    t = 2
    junk = np.stack([x1, np.array([-2.0 * lam, 1. + 4.0 * lam, - 2.0 * lam]), x1_rev])
    junk = np.linalg.solve(junk.T, y[0:3])
    y_trend[t] = junk[t]

    t = 3
    i_begin = np.array([1, 1, 1, 2, 2, 2, 2])
    j_begin = np.array([1, 2, 3, 1, 2, 3, 4])
    x_begin = np.append(x1, x2)

    i_end = np.array([3, 3, 3, 3, 4, 4, 4])
    j_end = np.array([1, 2, 3, 4, 2, 3, 4])
    x_end = np.append(x2_rev, x1_rev)

    matrix = np.zeros([4, 4])
    
    for l in np.arange(len(np.append(i_begin, i_end))):
        i = np.append(i_begin, i_end)[l] - 1
        j = np.append(j_begin, j_end)[l] - 1
        matrix[i, j] = np.append(x_begin, x_end)[l]

    y_trend[t] = np.linalg.solve(matrix, y[0:4])[t]

    for tc in np.arange(4, T):
        i_begin = np.append(i_begin, np.tile(tc - 1, 5))
        j_begin = np.append(j_begin, np.arange(tc - 3, tc + 2))
        x_begin = np.append(x_begin, x3)
        i_end += 1
        j_end += 1

        matrix = np.zeros([tc + 1, tc + 1])
        irange = np.array(i_begin.tolist() + i_end.tolist())
        jrange = np.array(j_begin.tolist() + j_end.tolist())
        xses = np.array(x_begin.tolist() + x_end.tolist())
        for l in range(len(irange)):
            i = irange[l] - 1
            j = jrange[l] - 1
            matrix[i, j] = xses[l]
        y_trend[tc] = np.linalg.solve(matrix.T, y[0:tc + 1])[tc]
